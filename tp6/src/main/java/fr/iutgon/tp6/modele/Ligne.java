...
public class Ligne {
  ...
  public Ligne(int qte, Produit produit) {
	...
	this.totalHT = calcPrixHT();
	this.totalTTC = calcPrixTTC();
  }
  ...
  private NumberExpression calcPrixHT() {
	  /** 
	   * version de Mr. JEANPIERRE (compile-safe)
	   * 
		  FloatProperty valeur = new SimpleFloatProperty();
		  valeur.bind(getProduit().prixProperty());
		  produitProperty().addListener(obs->{
			  valeur.unbind();
			  valeur.bind(getProduit().prixProperty());
		  });
		  return Bindings.multiply(qteProperty(), valeur);
	  */
	  return Bindings.multiply(this.qte, Bindings.selectFloat(this.produitProperty(), "prix"));
  }
  
  private NumberExpression calcPrixTTC() {
	  return Bindings.multiply(this.totalHT, Bindings.selectFloat(this.produitProperty(), "tva"));
  }
